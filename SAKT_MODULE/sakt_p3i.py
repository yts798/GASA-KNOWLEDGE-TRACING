...
from datareaderp3 import readerp3
...
from utils import collate_fn_p3
...
class SAKT_P3I(KTStrategy):
    ...
    def run_model(self):
        ...
        dataset = readerp3(seq_len, self.data_name)
        ...
        model = SAKT_Model(dataset.num_q, dataset.num_a, dataset.num_b, dataset.num_c, **model_config).to(device)
        ...
        train_loader = DataLoader(
            ...
            collate_fn=collate_fn_p3, generator=torch.Generator(device=device)
...
#
#
# class EncoderEmbeddingCat(Module):
#     def __init__(self, num_a, num_b, num_c, num_emb, d):
#         super().__init__()
#         self.num_a = num_a
#
#         self.num_b = num_b
...
#         self.b_eye = torch.eye(num_b)
#         self.c_eye = torch.eye(num_c)
#         self.cemb = Linear(num_a + num_b+num_c, num_emb, bias=False)
#         self.num_emb = num_emb
#         self.d = d
#         self.ea = Embedding(self.num_a*2, self.num_emb)
#         self.eb = Embedding(self.num_b*2, self.num_emb)
#         self.ec = Embedding(self.num_c*2, self.num_emb)
#         self.eq = Linear(self.num_emb, self.d)
#         self.test = Embedding(self.num_emb, self.num_a)
#
#         a_e = self.a_eye[a].to(device)
#         b_e = self.b_eye[b].to(device)
#         ct = torch.cat((a_e,b_e), -1) # bz * seq_len * num_fea
#
#         Cct = self.cemb(ct)
#         theta = torch.mul(y_e, Cct)
#         theta = torch.cat((theta, ct), -1)
#     def forward(self, a, b, c, r):
#         ax = a + self.num_a * r
#         bx = b + self.num_b * r
#         cx = c + self.num_c * r
#         ax = self.ea(ax)
...
#     def __init__(self, num_a, num_b, emb_size) -> None:
#         super().__init__()
#         self.a_eye = torch.eye(num_a)
#         self.b_eye = torch.eye(num_b)
#         self.cemb = Linear(num_a + num_b, emb_size, bias=False)
#
#     def forward(self, y_e, a, b):
#         a_e = self.a_eye[a].to(device)
#         b_e = self.b_eye[b].to(device)
#         ct = torch.cat((a_e,b_e), -1) # bz * seq_len * num_fea
#
...
class SAKT_Model(Module):
    ...
    '''
    def __init__(self, num_q, num_a, num_b, num_c, n, d, num_attn_heads, dropout):
        super().__init__()
        self.num_q = num_q
        self.num_a = num_a
        self.num_b = num_b
        ...
        self.EA = EncoderEmbedding(self.num_a, self.num_b, self.num_c, 128 , d)
    ...
    def forward(self, q, r, qry, a, b, c):
        '''
            Args:
        ...
        Madd = self.EA(a.long(), b.long(), c.long(), r.long()).permute(1, 0, 2)
        # print(M.size(), Madd.size())
        M = M + Madd
    ...
    def train_model(
        ...
        for i in range(1, num_epochs + 1):
            ...
            for data in train_loader:
                q, r, qshft, rshft, a, ashft, b, bshft, c, cshft, m = data

                self.train()

                p, _ = self(q.long(), r.long(), qshft.long(), a.long(), b.long(), c.long())
