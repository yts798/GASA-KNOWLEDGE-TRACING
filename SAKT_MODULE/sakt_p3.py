...
class SAKT_P3(KTStrategy):
    ...
    def run_model(self):
        ...
        print(aucs)
        print(loss_means)
        with open(os.path.join(ckpt_path, "aucs.pkl"), "w") as f:
            pickle.dump(aucs, f)
        with open(os.path.join(ckpt_path, "loss_means.pkl"), "w") as f:
...
class SAKT_Model(Module):
    ...
    def __init__(self, num_q, num_a, num_b, num_c, n, d, num_attn_heads, dropout):
        ...
        self.Ma = Embedding(self.num_a * 2, self.d)

        self.Mb = Embedding(self.num_b * 2, self.d)
        self.Mc = Embedding(self.num_c * 2, self.d)
    ...
    def forward(self, q, r, qry, a, b, c):
        ...
        Ma = self.Ma(ax).permute(1, 0, 2)
        Mb = self.Mb(bx).permute(1, 0, 2)
        Mc = self.Mc(cx).permute(1, 0, 2)
        M = M + Ma
        M = M + Mb
        M = M + Mc
